...
let handleWeatherResponse = function(response) {
  ...
  $(".forecast").empty();
    ...
    $("#current-conditions-text").append(currentWeather.summary);

    for (let i=0; i<6; i++) {
      let forecastWeather = response.daily.data[i];
        let html = '<div class="col-4">';
          html = html + '<h3>' + icon(forecastWeather.icon) + '</h3>';
          html = html + '<h4>' + Math.round(forecastWeather.temperatureHigh) + ' | ' + Math.round(forecastWeather.temperatureLow) + '</h4>';
          html = html + '<h5>' + forecastWeather.summary + '</h5>';
          html = html + '</div>';
